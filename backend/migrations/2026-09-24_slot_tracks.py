...
def main():
    ...
    for table, (old_uq, new_uq, cols) in PLAN.items():
        ...
        if is_sqlite:
            ...
            # rebuilt. Local SQLite databases are disposable, so say so rather
            # than doing surgery nobody needs.
            done.append(
        ...
        with engine.begin() as c:
            if old_uq in existing:
                c.execute(text(f"ALTER TABLE {table} DROP CONSTRAINT {old_uq}"))
            quoted = ", ".join(f'"{col}"' for col in cols)
            c.execute(text(f"ALTER TABLE {table} ADD CONSTRAINT {new_uq} UNIQUE ({quoted})"))
